...
export const getAssignments = async (req, res, next) => {
    try {
        const assignments = await surveyService.getAssignments({
            auditorId: req.user.role === "FIELD_AUDITOR" ? req.user.id : req.query.auditorId,
...
export const updateAssignmentStatus = async (req, res, next) => {
    try {
        const assignment = await surveyService.updateAssignmentStatus(req.params.id, req.body.status, req.user);
...
export const submitEntry = async (req, res, next) => {
    try {
        ...
        res.status(201).json({ status: "success", message: "Price recorded", data: { entry } });
    } catch (error) {
        next(error);
    }
};
...
export const syncBatch = async (req, res, next) => {
    try {
        const summary = await surveyService.syncBatchEntries(req.user.id, req.body.entries);
        res.status(200).json({ status: "success", message: "Batch synced", data: summary });
    } catch (error) {
        next(error);
    }
};
