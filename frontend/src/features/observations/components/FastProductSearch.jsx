...
export const FastProductSearch = ({
  ...
  // Exact barcode/SKU/id match → auto-select
  useEffect(() => {
    if (!query.trim()) return;
    const clean = query.trim();

    const exactMatch = products.find(
      (p) => p.barcode === clean || p.sku === clean || (p.productId || p.id) === clean
    );
    ...
    <div className="relative w-full">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-2xl border border-slate-300 bg-white py-3.5 pl-10 pr-24 text-sm font-medium text-slate-900 placeholder:text-slate-400 shadow-xs transition focus:border-[#A41821] focus:ring-2 focus:ring-[#A41821]/15 outline-hidden disabled:bg-slate-100"
        />
        ...
        <div className="pointer-events-none absolute inset-y-0 right-2 flex items-center gap-1.5">
          {query ? (
            <button
              ...
              tabIndex={-1}
              onClick={() => setQuery("")}
              className="pointer-events-auto p-1 text-xs text-slate-400 hover:text-slate-600"
            >
              ✕
            </button>
          ) : (
            <kbd className="hidden items-center gap-0.5 rounded-md border border-slate-200 bg-slate-50 px-1.5 py-0.5 font-mono text-[10px] font-bold text-slate-400 sm:inline-flex">
              ⌘K
            </kbd>
          )}
        </div>
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-xs transition ${
                      isHighlighted ? "bg-[#A41821] text-white" : "text-slate-700 hover:bg-slate-50"
                    }`}
                  >
                    <div className="min-w-0 flex-1 pr-2">
                      ...
                      >
                        CODE: {product.barcode || product.sku || prodId} • {product.category} (
                        {product.unit})
                      </div>
                    </div>
